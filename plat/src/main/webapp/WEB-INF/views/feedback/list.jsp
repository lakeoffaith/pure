...
<html lang="en">
  ...
  <body>
 <%--  action="<%=basePath %>feedback/list.action" --%>
      <form id="domainForm" Class="form-inline" action="feedback/list.action">
              <!-- page start-->
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12" style="margin: 0px 15px 0px">
                      <section class="panel">
                            ...
                            <tr>
                              <th colspan="10" class="oprow-btn">
                              	...
                              	<div class="col-lg-8 col-md-8 col-sm-8">
    								...
    								<input type="button" id="newInputDomainID" class="hidden"   onclick="doDomain('feedback','input')"></input>
    								<label for="batchDeleteDomainID" class="btn btn-danger " ><i class="icon-trash">&nbsp;批量删除</i></label>
    								<input type="button" id="batchDeleteDomainID" class="hidden"  style="margin-left:10px"  onclick="batchDeleteDomain('feedback')"></input>
    								</div>
    								...
    								<input type="button" id="searchDomainID" class="hidden" style="float: right"  onclick="doDomain('feedback','search')"></input>
    								</div>
    								</div>
                              </th>
                          </tr>
                          ...
                          <c:forEach  items="${pageResult.rows}"  var="r">
                              ...
                              <td>
                                  	<label for="viewDomainID" class="btn btn-success btn-xs" title="查看" onclick="doDomain('feedback','input','${r.id}')"><i class="icon-file"></i></label>
    								<input type="button" id="viewDomainID" class="hidden"   ></input>
                                   <label for="inputDomainID" class="btn btn-primary btn-xs" title="编辑" onclick="doDomain('feedback','input','${r.id}')"><i class="icon-pencil"></i></label>
    								<input type="button" id="inputDomainID" class="hidden"   ></input>
                                   <label for="deleteDomainID" class="btn btn-danger btn-xs" title="删除" onclick="doDomain('feedback','delete','${r.id}')"><i class="icon-trash"></i></label>
              ...
              <!-- page end-->
              <page:htmlPage pageNo="${pageResult.currentPage}" url='feedback/list.action' totalSum="${pageResult.totalCount}" showPage="5" pageSize="${pageResult.pageSize}"/>
          </form> 
